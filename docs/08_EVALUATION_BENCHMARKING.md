...
Comparing a mask run's leakage against a redact run's as if they were the same metric is forbidden; the two variants carry distinct names and the strategy travels with every result so the reading cannot be confused. Measured on the committed corpus, hybrid chain: full-value leakage is 0.067 under *both* strategies — masking covers the exact surface just as redaction does — while fragment leakage is 0.078 under `redact` and 0.567 under `mask`, which is the deliberate retention (`last4`, `partial_email`) becoming visible. Redact's 0.078 sits slightly *above* its own full-value 0.067 rather than equalling it, which it did before ADR-0020; that gap is two Greek names where a promoted span removes the surname and leaves the given name, and it is checked rather than assumed — no entity leaks that did not leak before promotion.
