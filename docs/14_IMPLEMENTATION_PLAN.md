...
> **The project reached `v0.1.0` (session 13, 2026-08-22), and session 14 opened a
> new line of work on top of it** — making the shipped engine's configuration
> reachable from the service, then from a UI. That is *additive*: `v0.1.0` remains a
> complete release, and everything below it is still true.
>
...
first second rather than after the run. **56 gates across three corpora; no published number moved.** The session-11 pickup list below resumes unchanged, and `docs/20` §9 adds four
...
| V1 | **ADR-0034: what a caller may choose.** The two settings most likely to change a result on a real column — `split_lines` (ADR-0016) and `preserve_prefix` (ADR-0032) — are reachable through `POST /configs`, from a template menu the operator opts into per option. The rule is **quality knobs yes, privacy boundaries never** — a rule that replaces a "can only redact more" formulation this repository's own measurements falsify. Typed `dict[str, bool]`, so a caller's choice is a selection and no free-form string reaches a parser. **Validity moved to `config/registries.py`** after review (`KNOWN_PARSER_OPTIONS`, pinned by equality), which fixed a pre-existing bug for *all four* entry points: a `parser_options` typo in a hand-written YAML used to survive config validation and die when the pipeline built the parser. **Both auditors found real defects again**, one of them a live leak: `parser_options` is the first request `dict`, and pydantic puts a rejected *key* into the error location — so an unbounded caller string was echoed verbatim in a 422 body, and the test that was supposed to catch it only exercised the *value* path. Fixed, both routes, both pinned; a second contract guard now walks every request field's annotation and refuses `Any`, `object` and unpatterned `str` | 1288 default (+48), 97 integration, **56/56 gates unchanged on both chains**; `ruff`, `mypy src tests` clean — no published number moved, no shipped default changed |

| V2 | **ADR-0035: the control panel ships with the service.** One static HTML file at `/` and `/ui`, on by default, `--no-ui` off — no build step, no CDN, byte-identical for every caller, `textContent` never `innerHTML`, no client-side storage, and it remembers nothing the server knows. ADR-0026 said a UI is a client; this answers *which* client. **Both auditors found real defects**: no framing headers on a page with two state-changing buttons (a hostile frame plus one tricked click from an authenticated operator triggers a run — now CSP + `X-Frame-Options: DENY`); a "ships in the wheel" test that compared a file to itself under an editable install, with packaging relying on hatchling's `.gitignore`-honouring default (now declared explicitly, with a `packaging`-marked test that builds a wheel and reads it back, run by CI); and the page keeping its own copy of `preserve_prefix: true` and "ADDRESS is undetected", which it *transmitted* — `/templates` now reports each option's engine default **and caption**, pinned against the parsers by value | 1311 default (+23), 97 integration, 1 packaging, 56 gates unchanged |
| V3 | **ADR-0036: a template may offer a directory.** `select_file: true` makes `source.path` a directory and the caller names one file in it — so an uploaded volume file becomes a run **without the service ever receiving the file**, and the caller still cannot name a source. The only place a caller string becomes part of a filesystem path, so: pattern (no separator, no `..`, no leading dot), resolve-and-contain (the symlink route), and two canonicalisation rules found by probing — **Windows device names** (`NUL`, `COM1` pass the pattern *and* containment) and **trailing dots** (Windows strips them, so two names map to one file), both refused on every platform. **Three audit findings**: the offer and the acceptance were different sets (the listing filtered by suffix, the builder did not — so a caller could name a `.txt` the listing withheld); the response echoed the joined absolute path three lines from where `saved_path` is deliberately relativized; and the `.gitignore` negation for `README.md` is case-insensitive, so `readme.md` in an upload bundle was committable | 1380 default (+69), 97 integration, **56/56 gates unchanged**; driven in a browser and re-verified over HTTP after every fix |
